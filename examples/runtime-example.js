...
function buildLibraryMetamodel() {
    ...
    // Create the package
    const libraryPackage = new (class extends runtime_1.BasicEPackage {
        constructor() {
            super();
    ...
    const libraryClass = new runtime_1.EClassBuilder('Library')
        .build();
    // Create Book class
    const bookClass = new runtime_1.EClassBuilder('Book')
        .build();
    // Create Writer class
    const writerClass = new runtime_1.EClassBuilder('Writer')
        .build();
    // Add attributes to Library
    const libraryName = new runtime_1.EAttributeBuilder('name', runtime_1.EcoreDataTypes.EString)
        .required()
        .build();
    libraryClass.addFeature(libraryName);
    // Add attributes to Book
    const bookTitle = new runtime_1.EAttributeBuilder('title', runtime_1.EcoreDataTypes.EString)
        .required()
        .build();
    const bookPages = new runtime_1.EAttributeBuilder('pages', runtime_1.EcoreDataTypes.EInt)
        .defaultValue('0')
        .build();
    const bookISBN = new runtime_1.EAttributeBuilder('isbn', runtime_1.EcoreDataTypes.EString)
        .id(true)
        .build();
    ...
    const writerName = new runtime_1.EAttributeBuilder('name', runtime_1.EcoreDataTypes.EString)
        .required()
        .build();
    writerClass.addFeature(writerName);
    // Add references
    const booksReference = new runtime_1.EReferenceBuilder('books', bookClass)
        .containment(true)
        .many(true)
        .build();
    libraryClass.addFeature(booksReference);
    const writersReference = new runtime_1.EReferenceBuilder('writers', writerClass)
        .containment(true)
        .many(true)
        .build();
    libraryClass.addFeature(writersReference);
    const authorReference = new runtime_1.EReferenceBuilder('author', writerClass)
        .build();
 ...
 */
function createInstances(metamodel) {
    console.log('=== Creating Instances ===\n');
    const { libraryPackage, libraryClass, bookClass, writerClass } = metamodel;
    // Create factory
    const factory = new runtime_1.BasicEFactory();
    ...
    console.log(`  Books: ${library.eGet(booksFeature).length}`);
    console.log(`  Writers: ${library.eGet(writersFeature).length}`);
    console.log();
 ...
 */
function demonstrateReflectiveAPI(library, metamodel) {
    ...
    for (const book of books) {
        ...
        const authorName = author ? author.eGet(author.eClass().getEStructuralFeature('name')) : 'Unknown';
 ...
 */
function demonstrateContainment(library) {
 ...
 */
async function demonstrateSerialization(library, metamodel) {
    console.log('=== Resource and Serialization ===\n');
    const { libraryPackage } = metamodel;
    // Create resource set
    const resourceSet = new runtime_1.BasicResourceSet();
    resourceSet.getPackageRegistry().set(libraryPackage.getNsURI(), libraryPackage);
    // Create resource
    const uri = URI_1.URI.createURI('library://my-library.json');

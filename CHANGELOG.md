...
- `getURIFragment()` produces the path fragment EMF defines ([#89](https://github.com/eclipse-fennec/emf.ts/issues/89)). A same-document reference was written as a bare index path (`/1/0`), which Java EMF cannot resolve — `eObjectForURIFragmentSegment()` requires the `@feature` form and reports `Expecting @ at index 0`. The fragment now names the containment feature per step: `/1/@operations.0`, `/1/@operations.0/@parameters.0`, and `/1/@main` without an index for a single-valued containment.
  ...
  The index was wrong on its own terms too: it counted across all containment children rather than within one feature, so a class with two containment features produced a number that meant something different on each side. It is computed within the feature now.

  Reading is unaffected — the `@feature.index` form was already supported, and documents written by earlier versions with a bare index path stay readable. `XMLResource` and `JSONResource` inherit the fix through their fallback to `BasicResource`. Two test expectations that pinned the old `/0/0` output were updated.
...
- The type of an unresolvable cross-document target survives a round trip ([#85](https://github.com/eclipse-fennec/emf.ts/issues/85), [#87](https://github.com/eclipse-fennec/emf.ts/issues/87)). `xsi:type` on a reference element was discarded on load — `createObject()` returned as soon as it saw an `href` — so the proxy carried the feature's declared type and the actual one was gone. It is read now and given to the proxy, and written back out. Since the prefix has to be declared for the next load to resolve it, proxy targets are included when collecting namespaces; without that the type degraded one step per round trip.
- Namespace prefixes used by reference types are declared ([#87](https://github.com/eclipse-fennec/emf.ts/issues/87)). `collectPackages()` walked contained objects only, so a prefix appearing solely in a reference stayed undeclared and the reader could not resolve it. The type of a cross-document target is written as `xsi:type` on the href element — where EMF puts it, and where the prefix is accounted for — instead of as a prefix inside the attribute value.
  ...
  Both required one shared criterion for "is this target in another document", which the attribute and element branches had answered differently: comparing `eResource()` misjudges a classifier reachable through the package chain, which often has no container of its own. The decision is made on the address form now — a bare fragment, path or IDREF addresses this document, anything carrying a base URI another one. Several expectations in `tests/XMISave.test.ts` pinned the previous output and were updated.
...
- A relative cross-document reference resolves against the referencing document ([#93](https://github.com/eclipse-fennec/emf.ts/issues/93)). `resolveReference()` passed the base part of `href="payment.xmi#/"` to the ResourceSet unchanged, which missed the resource held under its absolute URI and — with `loadOnDemand` — created an empty one under the relative name. The reference did not fail but came back pointing at the wrong object, typically a root of the referencing document itself, and saving wrote that state into the file, destroying the links between documents. `createProxy()` in the same file already resolved the base; `resolveReference()` does now too.
- Cross-document hrefs are written relative to the current document. `getHref()` emitted the target's absolute URI, which would bake the author's filesystem path into the file. It is deresolved against the referencing resource, as Java EMF does and as the proxy branch already did.
